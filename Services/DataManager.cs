...
public class DataManager
{
    ...
    public List<Entry> LoadEntries(DateTime date)
    {
        ...
        {
            string json = File.ReadAllText(path);
            return JsonConvert.DeserializeObject<List<Entry>>(json, _settings) ?? [];
        }
        catch (JsonException)
        {
            return [];
	...
	public List<Entry> GetAllEntries()
	{
		...
		if (File.Exists(TodayEntriesPath)) files.Add(TodayEntriesPath);

		if (Directory.Exists(EntriesDir))
		{
			files.AddRange(Directory.GetFiles(EntriesDir, "entries-*.json"));
		}

		return [.. files
			.Select(LoadEntriesFromFile)
			.SelectMany(x => x)];
	}

	// prywatna metoda pomocnicza - enkapsulacja logiki odczytu i obsługi błędów
	private List<Entry> LoadEntriesFromFile(string path)
	{
		try
		{
			string json = File.ReadAllText(path);
    ...
    {
        if (!File.Exists(MealTemplatesPath)) return [];
        try
        {
            string json = File.ReadAllText(MealTemplatesPath);
            return JsonConvert.DeserializeObject<List<MealTemplate>>(json, _settings) ?? [];
        }
        catch { return []; }
    }
    ...
    public void AddMealTemplate(MealTemplate template)
    {
        ...
        template.Id = templates.Count != 0 ? templates.Max(m => m.Id) + 1 : 1;
    ...
    {
        if (!File.Exists(ActivityTemplatesPath)) return [];
        try
        {
            string json = File.ReadAllText(ActivityTemplatesPath);
            return JsonConvert.DeserializeObject<List<ActivityTemplate>>(json, _settings) ?? [];
        }
        catch { return []; }
    }
    ...
    public void AddActivityTemplate(ActivityTemplate template)
    {
        ...
        template.Id = templates.Count != 0 ? templates.Max(a => a.Id) + 1 : 1;
